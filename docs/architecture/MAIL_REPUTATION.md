...
and delivery errors for `gulyaly.com` and `newsletter.gulyaly.com` separately:
1. Sign in with a Google account, add each domain (both `gulyaly.com` and
   `newsletter.gulyaly.com` — they're separate sending identities as of the mail-split work).
