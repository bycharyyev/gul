...
- **Two independent transporters**: "main" (transactional — order mail, REG.RU) and "marketing"
  ...
  (`news2026._domainkey.newsletter.gulyaly.com`), so spam complaints on a broadcast can't affect
...
| Login | full mailbox address, e.g. `noreply@gulyaly.com` |
...
**Applied in production** on both hosts. `MAIL_REPLY_TO` is set to `support@gulyaly.com`, since
...
`gulyaly.com`'s authoritative DNS is Timeweb (not REG.RU) — see `CLAUDE.md`'s "DNS management"
...
| Record | Value | Status |
|---|---|---|
| A `gulyaly.com` | `DEPLOY_HOST` (site/API, unrelated to mail) | unchanged |
| MX `gulyaly.com` | `10 mx1.hosting.reg.ru`, `15 mx2.hosting.reg.ru` | live (replaced the old unused `mx1/mx2.timeweb.ru` defaults) |
| SPF (apex TXT) | `v=spf1 include:_spf.timeweb.ru a:mail.gulyaly.com include:_spf.hosting.reg.ru ~all` | live (merged into the existing record, not a second SPF record) |
| DMARC `_dmarc.gulyaly.com` | `v=DMARC1; p=quarantine; pct=100; adkim=r; aspf=r` | unchanged, already in place |
| DKIM `dkim._domainkey.gulyaly.com` | 2048-bit key generated by ISPmanager | live (published manually — see below) |
...
only auto-publishes records when NS points at `ns1/ns2.hosting.reg.ru`. `gulyaly.com` stays on
Timeweb NS, so that path was unavailable.

What worked: registering `gulyaly.com` as a mail domain in ISPmanager with "Enable DKIM" ticked
...
`admin.gulyaly.com/mail` — settings, logs, test send, broadcast, suppression list, health.

`admin.gulyaly.com/mail/templates` — the template editor. Browse by kind and locale; edit
...
MAIL_USER=noreply@gulyaly.com MAIL_PASS=... SMTP_TEST_RECIPIENT=you@gmail.com \
...
3. **Old Postfix DKIM cleanup** — `mail._domainkey.gulyaly.com` (the self-hosted relay's key) is
