...
/// One full-screen post, exactly as the discover feed and a shop's own grid both render it --
/// extracted so a shop page can open the same swipeable view over its own posts instead of a
/// bespoke reader that could drift from what the feed actually looks like.
///
/// Reactions are handed in rather than reached for, because "which list does this post live in"
/// is the caller's question: the discover feed reorders and pages on its own controller, a shop's
/// grid keeps its own -- both know how to replace one post in their own state, and this widget
...
void prefetchPostsAround(List<SocialPost> posts, int index) {
...
class _FeedVideoState extends State<_FeedVideo> with WidgetsBindingObserver {
  ...
  /// downloaded here -- the feed prefetches ahead (see `prefetchPostsAround`), so a video reached
  /// by swiping is usually already a file by the time this runs.
